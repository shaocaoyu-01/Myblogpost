...
![Orcas distribution exploration](../../images/orcas.jpg){#fig-variant-c}
...
ggplot(monthly_counts,aes(x= month,y = encounters))+
  geom_line() +
  labs(
    title = "Figure 2. Monthly orcas encounters",
    x = "Time" , 
    y = "Encounter_times")+
...
ggplot(monthly_counts, aes(x = m_lab, y = encounters)) +
  geom_boxplot() +
  labs(title = "Figure 3. Seasonality by month (counts across years)", 
       x = NULL, 
       y = "Encounters")+
   theme_minimal(base_size = 12)
...
ggplot(Sep_share_on_year,aes(x=year,y=proportion))+
         ...
         labs(title = "Figure 4.Proportion of encounters in September by year",
              x= "Year",
...
duration_time_df <- duration_time_df |>
  ...
  labs(
    title = "Figure 5.Encounter duration distribution by 1000s bins",
    x = "Duration",
    y = "Encounters"
  )+theme_minimal(base_size = 12)
```
...
p_points <-
  ggplot(enc_sf) +
  geom_sf() +
  labs(title = "Figure 6.Orca Encounters: Overall Spatial Distribution") +
  theme_minimal(base_size = 12)
...
p_hex <-
  ...
  labs(
    title = sprintf("Figure 7.Encounter Hotspots (Hex ~%d km)", HEX_SIZE_M/1000),
    fill  = "Count"
  ) +
  theme_minimal(base_size = 12)
...
obs_long <- df %>%
  ...
  separate_rows(observers, sep = "\\s*(?:,|;|/|\\||&|\\+|、|，|\\band\\b)\\s*",
                convert = FALSE) %>%

  mutate(observers = str_squish(observers),
         observers = str_remove_all(observers, "^[-–•]+|[-–•]+$"),
...
top_observers %>%
  ...
  geom_col() +
  coord_flip() +
  labs(title = "Figure 8.Most Frequent Observers", x = NULL, y = "Count") +
  theme_minimal(base_size = 12)
...
#visualize
  ...
  ggplot(top10_v, aes(x = reorder(.data[[col_vessel]], n), y = n)) +
    geom_col() +
    coord_flip() +
    labs(title = "Figure 9.Top 10 active vessels by number of encounters",
...
ggplot(top_words %>% slice_max(n, n = 20),
  ...
  geom_text(aes(label = n), 
            ...
            size = 2.5) + 
  coord_flip() +
  labs(title = "Figure 10.Top words in encounter descriptions",
       x = NULL, y = "Count") +
  theme_minimal(base_size = 12)
```
...
<https://CRAN.R-project.org/package=dbscan>.
  ...
  R package version 1.1.9, <https://CRAN.R-project.org/package=ggspatial>.
   ...
   and Cédric Scherer (2024). viridis(Lite) - Colorblind-Friendly Color Maps for R.
  ...
  Data Principles in R.” _JOSS_, *1*(3). doi:10.21105/joss.00037
